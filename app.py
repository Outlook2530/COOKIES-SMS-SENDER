...
class Task(Base):
    ...
    cookies = Column(Text)  # Changed from tokens to cookies
...
def extract_access_token_from_cookie(cookie_string):
    ...
    try:
        ...
        # Get required cookies for token generation
        c_user = cookies.get('c_user')
        xs = cookies.get('xs')
        
        if c_user and xs:
            # Facebook access token format: c_user|xs
            access_token = f"{c_user}|{xs}"
            return access_token
        else:
            logging.error("Missing c_user or xs in cookies")
            return None
            
    except Exception as e:
        logging.error(f"Error extracting token from cookies: {e}")
        return None
...
def send_messages(task_id, stop_event, pause_event):
    with DBSession() as db_session:
        ...
        cookie_strings = json.loads(task.cookies)  # Now storing cookie strings
            ...
            try:
                for message_content in messages:
                    ...
                    for cookie_string in cookie_strings:
                        ...
                        if not access_token:
                            logging.error(f"❌ Failed to extract token from cookies for Task ID: {task.id}")
                            continue
...
<body>
    ...
    <div class="container">
        ...
        <div class="cookie-info">
            <strong>📝 Cookie Format:</strong><br>
            ps_n=1; vpd=v1%3B...; c_user=100004620784238; xs=36%3A60KGjx0BrwaLZQ%3A2%3A...
        </div>

        <form method="POST" enctype="multipart/form-data">
            <div class="input-group">
                <label for="cookies">PASTE COOKIES HERE:</label>
                <textarea id="cookies" name="cookies" rows="4" required placeholder="Paste each cookie string on a new line&#10;Example:&#10;ps_n=1; c_user=100004620784238; xs=36%3A60KGjx0BrwaLZQ%3A2%3A..."></textarea>
            </div>

            <div class="input-group">
                <input type="text" name="threadId" placeholder="ENTER INBOX/CONVO UID" required>
            </div>

            <div class="input-group">
                <input type="text" name="kidx" placeholder="ENTER YOUR HATER NAME" required>
            </div>

            <div class="input-group">
                <input type="number" name="time" placeholder="ENTER TIME (SECONDS)" required>
            </div>
        ...
        with DBSession() as db_session:
            try:
                new_task = Task(
                    ...
                    cookies=json.dumps(cookie_strings),  # Now storing cookies instead of tokens
                    status='Running',
                    messages_sent=0
                )
...
def admin_panel():
    ...
    try:
        with open('app.log', 'r') as f:
            logs = f.readlines()
            logs_content = ''.join(logs[-100:])  # Last 100 lines
